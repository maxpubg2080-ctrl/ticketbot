...
def find_file(base_name):
    """Faylni ham .png, ham .jpg, ham .jpeg ko'rinishida qidiradi"""
...
def get_airline_logo(airline_name):
    ...
    if 'khiva' in name:
        ...
        if path: return Image(path, width=110, height=30)
    elif 'centrum' in name:
        path = find_file('centrum')
        if path: return Image(path, width=120, height=28)
    elif 'uzbekistan' in name or 'uzairways' in name or 'hy' in name:
        path = find_file('uzairways') or find_file('uzbekistan')
        if path: return Image(path, width=120, height=28)
        
    return Paragraph(f"<b>✈️ {airline_name.upper()}</b>", ParagraphStyle('A', fontName=FONT_BOLD, fontSize=10, alignment=1))
...
def build_pdf_confirmation(data, filename):
    ...
    doc = SimpleDocTemplate(
        filename, pagesize=A4, rightMargin=20, leftMargin=20, topMargin=15, bottomMargin=15
    )
    story = []
    ...
    style_title = ParagraphStyle('T', parent=styles['Normal'], fontName=FONT_BOLD, fontSize=13, alignment=1, textColor=PRIMARY_BLUE, spaceAfter=8)
    style_lbl_bold = ParagraphStyle('LB', parent=styles['Normal'], fontName=FONT_BOLD, fontSize=8.5, textColor=PRIMARY_BLUE)
    style_lbl = ParagraphStyle('L', parent=styles['Normal'], fontName=FONT_NAME, fontSize=8, leading=10)
    style_c_bold = ParagraphStyle('CB', parent=styles['Normal'], fontName=FONT_BOLD, fontSize=8.5, alignment=1, textColor=PRIMARY_BLUE)
    style_c = ParagraphStyle('C', parent=styles['Normal'], fontName=FONT_NAME, fontSize=8, alignment=1, leading=10)
    style_disc = ParagraphStyle('D', parent=styles['Normal'], fontName=FONT_NAME, fontSize=6.5, leading=8.5, textColor=colors.HexColor('#475569'))

    story.append(Paragraph("ПОДТВЕРЖДЕНИЕ №", style_title))

    from_parts = data['from_city'].split()
    to_parts = data['to_city'].split()
    dep_code = from_parts[0] if len(from_parts) > 1 and len(from_parts[0]) == 3 else ""
    dep_city_name = " ".join(from_parts[1:]) if dep_code else data['from_city']
    arr_code = to_parts[0] if len(to_parts) > 1 and len(to_parts[0]) == 3 else ""
    arr_city_name = " ".join(to_parts[1:]) if arr_code else data['to_city']
    route_title = f"{dep_city_name} - {arr_city_name}".strip(" -")

    # Grand Turan logotipi (.jpg yoki .png bo'lishidan qat'i nazar topadi)
    gt_path = find_file('grandturan')
    if gt_path:
        gt_logo = Image(gt_path, width=100, height=50)
    else:
    ...
    # Туристы
    t_hdr = Table([[Paragraph("<u>СПИСОК ТУРИСТОВ:</u>", style_lbl_bold)]], colWidths=[555])
    t_hdr.setStyle(TableStyle([('BACKGROUND', (0, 0), (-1, -1), BG_CYAN), ('PADDING', (0, 0), (-1, -1), 3)]))
    story.append(t_hdr)

    title_gender = format_title_gender(data['gender'])

    tourist_data = [
        [Paragraph("№", style_c_bold), Paragraph("Ф.И.О", style_c_bold), Paragraph("Пол", style_c_bold), Paragraph("Паспорт", style_c_bold), Paragraph("Дата рождения", style_c_bold)],
        [Paragraph("1", style_c), Paragraph(f"<b>{data['name']}</b>", style_c), Paragraph(title_gender, style_c), Paragraph(data['passport'], style_c), Paragraph(data['birth_date'], style_c)]
    ]
    ...
    # Транспорт
    tr_hdr = Table([[Paragraph("<u>ТРАНСПОРТ:</u>", style_lbl_bold)]], colWidths=[555])
    tr_hdr.setStyle(TableStyle([('BACKGROUND', (0, 0), (-1, -1), BG_CYAN), ('PADDING', (0, 0), (-1, -1), 3)]))
    story.append(tr_hdr)

    dep_img_path = find_file('icon_dep')
    bag_img_path = find_file('icon_bag')
    
    dep_img = Image(dep_img_path, width=12, height=12) if dep_img_path else Paragraph("✈️", style_c)
    bag_img = Image(bag_img_path, width=12, height=12) if bag_img_path else Paragraph("🧳", style_c)

    h_dep = Table([[dep_img, Paragraph("<b>Отправление</b>", style_c_bold)]], colWidths=[15, 110])
    h_arr = Table([[dep_img, Paragraph("<b>Прибытие</b>", style_c_bold)]], colWidths=[15, 110])
    h_bag = Table([[bag_img, Paragraph("<b>Класс/Багаж</b>", style_c_bold)]], colWidths=[15, 110])

    airline_cell = [
        get_airline_logo(data['airline']),
        Spacer(1, 2),
        Paragraph(f"<font color='#475569'><b>{data['flight_code']}</b></font>", style_c)
    ]

    dep_text = f"<b>{dep_code}</b><br/>{dep_city_name}" if dep_code else f"<b>{dep_city_name}</b>"
    dep_cell = [
    ...
    arr_text = f"<b>{arr_code}</b><br/>{arr_city_name}" if arr_code else f"<b>{arr_city_name}</b>"
    arr_cell = [
...
async def handle_input(message: types.Message):
    ...
    # Ma'lumotlarni to'g'ri ajratib olish
    data = {
        'name': lines[0],
        'gender': lines[1],
        'passport': lines[2],
    ...
    try:
        ...
        await message.answer_document(doc_file, caption=f"✅ {data['name']} uchun PDF tayyor!")
